<template>
  <div id="All">
    ...
    <div id="Maincontainer">
      ...
      <div id="ResponseBox">
        ...
        align="left">
          ...
          <div id="ResponseParent">
            ...
            <textarea readonly v-show="ResponseTypeSelect == 'Cookie'" id="ResponseContent" v-model="responseCookie"></textarea>
            <textarea readonly v-show="ResponseTypeSelect == 'Header'" id="ResponseContent" v-model="responseHeader"></textarea>
          </div>
        </div>
      </div>
      </template>
      ...
      <details id="Right">
        <summary style="width:6vw;  font-weight:bold; cursor:pointer;">호출 로그</summary>

         <div style="width:30vw;" v-for="(log, index) in logList" v-bind:key="index">
...
export default {
  ...
  watch: {
    responsedata(newdata){
      ...
      else {   //실패했을 경우(ex : 404)의 response 처리
        this.res = newdata;
        this.res = JSON.stringify(this.res);
  ...
  "userPassword": "asdf"
          ...
          this.logList = [Log1, Log2, Log3, Log3, Log3, Log3, Log3, Log3, Log3, Log3, Log3, Log3, Log3];
        } else if (newindex == 4){
          Log3 = new Object();
          Log3.time = "2022.11.11 16:18";
          Log3.status = "200";
